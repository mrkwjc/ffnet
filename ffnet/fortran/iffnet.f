...
c
c************************************************************************
      subroutine prop(x, conec, n, outno, o, units, u)
c************************************************************************
c
c.....Gets conec and units with input already set 
c.....and calculates all activations.
c.....Identity input and output and sigmoid activation function 
c.....in hidden neurons is assumed
c
      implicit none
c.....variables
      integer n, u, o, conec(n,2), outno(o)
      double precision x(n), units(u), usum(u)
c.....helper variables
      integer src, trg, ctrg, xn, j
c.....f2py statements
...
c.....propagate signals with sigmoid activation function
      if (n.gt.0) then
          ...
          do xn=1,n
              ...
              !if next unit
              if (trg.ne.ctrg) then
                  units(ctrg) = 1./(1.+exp(-usum(ctrg)))
                  ctrg = trg
                  if (src.eq.0) then !handle bias
                      usum(ctrg) = x(xn)
                  else
                      usum(ctrg) = units(src) * x(xn)
                  endif
              else
                  if (src.eq.0) then !handle bias
                      usum(ctrg) = usum(ctrg) + x(xn)
                  else
                      usum(ctrg) = usum(ctrg) + units(src) * x(xn)
                  endif
              endif
          enddo
          do j=1,o
              units(outno(j)) = usum(outno(j)) !identity output
          enddo
...
c.........set input units
          ...
          enddo
c.........propagate signals
          call prop(x, conec, n, outno, o, units, u)
...
c.........set diffs at network output as back network inputs
          do k=1,o
	         ...
	         deriv = 1d0 !identity output !units(outno(k)) * (1. - units(outno(k))) !ugly
...
c.....and identity input)
      call prop(x, conec, n, outno, o, units, u)
      do k=1,u
	      units(k) = units(k) * (1d0 - units(k)) !all units as sigmoids
      enddo
      do k=1,i
         units(inno(k)) = 1d0
      enddo
c.....prepare output units for derivative network
      do k=1,o
          units(outno(k)) = 1d0 !identity output
          dunits(outno(k)) = 0d0
      enddo
c.....loop over inputs
      do di=1,i
